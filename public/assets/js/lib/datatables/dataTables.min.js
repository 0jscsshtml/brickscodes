...
document.addEventListener('DOMContentLoaded', ()=> {
	saveFormSubmissionFn()
});

function saveFormSubmissionFn() {
	const allSaveForms = document.querySelectorAll('.bc-form-table > table');
	if ( allSaveForms && allSaveForms.length > 0 ) {
		allSaveForms.forEach((table) => {
			if ( table.closest('.brxe-bc-form-submission[data-bc-advance-table]') ) {
				const docBody = document.body.classList.contains('bricks-is-frontend');
				const isResponsive = table.hasAttribute('data-table-responsive');
				const isExport	   = table.hasAttribute('data-table-export');
				const isColReorder = table.hasAttribute('data-table-colreorder');
				const elementId    = table.closest('.brxe-bc-form-submission[data-bc-advance-table]').getAttribute('data-script-id');
				...
				if ( docBody && table.closest('.brxe-bc-form-submission[data-bc-advance-table][data-frontend]') ) {
					if (isColReorder) {				
					...
					if (isExport) {
						dataTableOptions.layout.topEnd = {};
						dataTableOptions.layout.topEnd.buttons = [{extend: 'pdf', orientation: 'landscape', pageSize: 'A3'}, 'csv', 'excel' ];
						dataTableOptions.layout.topEnd.search = {
							placeholder: 'Type search here'
						}
					}
				}
				
				let dt = new DataTable(table, dataTableOptions);
				table.closest('.brxe-bc-form-submission[data-bc-advance-table]').style.opacity = 1;
				dt.on('responsive-display', function (e, datatable, row, showHide, update) {
					if ( showHide ) {
						const elementId = e.target.closest('.brxe-bc-form-submission[data-bc-advance-table]').getAttribute('data-script-id');
						document.body.querySelector('table.' + elementId + '').closest('.dtr-modal').classList.add(elementId);
					}
				});
			}
		});
